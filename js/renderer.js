...
// ═══════════════════════════════════════════════════════════════════════════
// HERO IMAGE PRE-PROCESSING (FOR SHARPNESS & PERFORMANCE)
// ═══════════════════════════════════════════════════════════════════════════

window.preprocessHeroImage = function(img) {
  if (!img) return;
  
  // Create high-res offscreen canvas for processing
  ...
  // Apply background removal ONCE
  if (typeof removeStudioBackground === 'function') {
    removeStudioBackground(ctx, canvas);
  }
  
  // Cache the resulting canvas in state
  S.processedHeroImage = canvas;
  console.log('✓ Hero image pre-processed and cached for sharpness');
};
// ═══════════════════════════════════════════════════════════════════════════
// UNIFIED CHARACTER PLACEMENT ENGINE (FOR STABILITY & RESOLUTION SYNC)
// ═══════════════════════════════════════════════════════════════════════════

/**
 * Calculates the 'Real' pixel Y position of the character's feet,
 * accounting for ground plane, damping, and the 'Headroom Safety Clamp'.
 */
window.calculateVisibleFeetY = function(h, groundY, charH) {
    const rawFeetY = h * Math.min(Math.max(0.1, groundY), 0.95);
    const headY = rawFeetY - charH;
    
    // Safety Clamp: Don't let the head go above 8% from the top
    const minHeadY = h * 0.08;
    if (headY < minHeadY) {
        return charH + minHeadY;
    }
    return rawFeetY;
};
 ...
 * Ensures that all rendering and stability math use identical subject dimensions.
 */
window.getSubjectMetrics = function(h, canvasRatio) {
  const charScale = canvasRatio >= 1.7 ? 0.72 : 0.82;
  const charH = h * charScale;
  return { charScale, charH };
};
  ...
  reader.onload = function(evt) {
    ...
    img.onload = async function() {
      ...
      try {
        ...
        if (result.foreground || result.midground || result.background || result.horizonY) {
          // Restore hVal/gVal parsing (accidentally removed)
          const hVal = parseFloat(result.horizonY);
          const gVal = parseFloat(result.groundY);

          // 🔥 PERMANENT STABILITY: Sync with unified placement and headroom clamp
          const aspectData = ASPECT_RATIOS[S.aspectRatio] || ASPECT_RATIOS['16:9'];
          const canvasRatio = aspectData.ratio;
          
          const hNorm = 1.0; // Use normalized height for resolution-independent math
          const { charScale } = window.getSubjectMetrics(hNorm, canvasRatio);
          const charHNorm = hNorm * charScale;
          
          const curVisibleFeetY = window.calculateVisibleFeetY(hNorm, S.groundPlaneY, charHNorm);
          const newGroundY = !isNaN(gVal) ? Math.max(0.1, Math.min(gVal, 0.95)) : S.groundPlaneY;
          const newVisibleFeetY = window.calculateVisibleFeetY(hNorm, newGroundY, charHNorm);
          
          // Find change in VISIBLE percentage
          const shiftPercentage = curVisibleFeetY - newVisibleFeetY;
          
          // Apply percentage-based offset AND update ground plane in a tight loop
          // to minimize render flicker
          S.charYOffset = (S.charYOffset || 0) + shiftPercentage;
          if (!isNaN(hVal)) S.horizonY = Math.max(0.1, Math.min(hVal, 0.9));
          S.groundPlaneY = newGroundY;

          // RESTORED: Populate the layer descriptions
          if (result.foreground) {
            const sjInput = document.getElementById('subjectInput');
            if (sjInput) sjInput.value = result.foreground;
            S.subjectLayer = result.foreground; // Corrected from foregroundLayer
          }
          if (result.midground) {
          ...
          renderFrames();
          generatePrompt();
          showToast('✓ AI ANALYZED: Scene layers & stability mapped');
        } else {
...
async function renderFrameForStoryboard(frameType) {
  return new Promise((resolve) => {
    ...
    const feetX = w / 2;
    const groundY = Math.max(0.1, Math.min(S.groundPlaneY || 0.88, 0.95));
    const feetY = window.calculateVisibleFeetY(h, groundY, charH);
    
    // 🔥 Percentage-Based Offsets for Storyboard Fallback
    const charX = feetX - (charW / 2) + (S.charXOffset || 0) * w;
    const charY = feetY - charH + (S.charYOffset || 0) * h;
...
window.drawBackgroundWithSimpleDOF = function(ctx, bgImage, x, y, w, h, groundY, maxBlur, timeFilter, angle, focusDepth = 0.45) {
  if (maxBlur <= 1) { // 🛡️ optimization: No need for complexity if sharp
    ctx.save();
    if (angle) {
    ...
    ctx.filter = (timeFilter !== 'none') ? `${timeFilter} blur(${maxBlur}px)` : 'none';
  ...
  ctx.save();
  const baseBlur = Math.min(maxBlur * 0.1, 1); 
  ctx.filter = (timeFilter !== 'none') ? `${timeFilter} blur(${baseBlur}px)` : `blur(${baseBlur}px)`;
  ctx.drawImage(bgImage, x, y, w, h);
  ctx.restore();
  ...
  // 3. APPLY PROPORTIONAL DEPTH MASK (Reset to Screen Identity)
  bCtx.setTransform(1, 0, 0, 1, 0, 0); 
  bCtx.globalCompositeOperation = 'destination-in';
  
  // Create a symmetric proportional gradient (locked to monitor frame)
  const grad = bCtx.createLinearGradient(0, 0, 0, canvasH);
  
  // Calculate focal slice
  const transitionSize = Math.max(0.05, focusDepth * 0.3); // Sharp-to-Blur falloff width
  const sharpTop = Math.max(0, groundY - (focusDepth / 2));
  const sharpBot = Math.min(1.0, groundY + (focusDepth / 4)); // Perspective shift: foreground gets blurry slower
  
  grad.addColorStop(0, 'rgba(0,0,0,1)');                   // Far Background: FULL BLUR
  grad.addColorStop(Math.max(0, sharpTop - transitionSize), 'rgba(0,0,0,1)'); 
  grad.addColorStop(sharpTop, 'rgba(0,0,0,0)');            // START SHARP
  grad.addColorStop(sharpBot, 'rgba(0,0,0,0)');            // END SHARP
  grad.addColorStop(Math.min(1.0, sharpBot + transitionSize), 'rgba(0,0,0,0.4)'); // Foreground transition
  grad.addColorStop(1, 'rgba(0,0,0,0.6)');                 // Near Foreground: SLIGHT BLUR
...
function drawCharacterWithShadow(ctx, img, x, y, width, height, timeOfDay, lightingStyle = '', sunDirection = 'east') {
    ...
    if (timeOfDay >= 8 && timeOfDay <= 16) {
        ...
        charFilter = 'none'; // 🔥 SHARPNESS FIX: Removed redundant 1.0 filters to prevent browser blur
    } else if (timeOfDay >= 6 && timeOfDay <= 18) {
        lightAngle = (timeOfDay - 12) / 6; 
        shadowLength = Math.abs(lightAngle); 
        charFilter = 'brightness(1.02) contrast(1.05) sepia(0.2) saturate(1.2)'; 
    } else {
        lightAngle = 0.3;
        shadowLength = 0.7;
        shadowOpacity = 0.4; 
        charFilter = 'brightness(0.65) contrast(1.1) saturate(0.6)'; 
    }
    ...
    if (lightingStyle.includes('Hard Shadow') || lightingStyle.includes('Noir')) {
        ...
        charFilter = (charFilter === 'none' ? '' : charFilter + ' ') + 'grayscale(0.4) contrast(1.3)';
        ...
        shadowLength = 1.2; 
        shadowOpacity = 0.9;
        charFilter = (charFilter === 'none' ? '' : charFilter + ' ') + 'brightness(0.5) contrast(1.2)';
    }

    const skewX = lightAngle * -1.2; 
    const scaleY = -0.15 - (shadowLength * 0.85); 

    // 2.5 THE CONTACT SHADOW (Grounding layer)
    ctx.save();
    ctx.translate(Math.round(x + footX), Math.round(y + footY)); // 🔥 PIXEL SNAP
    ...
    ctx.drawImage(img, Math.round(x), Math.round(y), width, height);
    ctx.restore();
}
...
function renderExportFrame(canvas, img, settings) {
  ...
  // 2. THE GROUND ANCHOR (Unified engine)
  const feetX = w / 2;
  const groundY = Math.max(0.1, Math.min(settings.groundPlaneY || S.groundPlaneY || 0.88, 0.95));
  const feetY = window.calculateVisibleFeetY(h, groundY, charH);
  ...
  // 🔥 FIX: Apply percentage-based offsets to export engine
  ...
  if (S.backgroundPlate) {
    ...
    const { maxBlur, focusDepth } = getBlurAmount(settings.dof || S.dof);
    let totalBlur = (maxBlur * 1.5) + opticalBlur;
    let currentFocusDepth = focusDepth;
    let fgBlur = 0;
  ...
  if (!skipCharacter && img) {
    ...
    // 🔥 FIX: Re-incorporating manual character offsets (formerly ignored)
    const finalCharX = Math.round(adjustedFeetX - (finalCharW / 2) + (moveX * 1.4));
    const finalCharY = Math.round(adjustedFeetY - finalCharH + moveY);
 ...
 */
/**
 * UNIFIED OPTICAL PHYSICS ENGINE
 * Maps DOF settings (f-stops) to physically plausible blur and depth metrics.
 * @returns {Object} { maxBlur: number, focusDepth: number }
 */
window.getBlurAmount = function(dof) {
  const settings = {
    'f/1.4 Razor Thin':   { maxBlur: 24,  focusDepth: 0.08 },
    'f/2.0 Shallow':      { maxBlur: 16,  focusDepth: 0.15 },
    'f/2.8 Cinematic':    { maxBlur: 10,  focusDepth: 0.22 },
    'f/4.0 Balanced':     { maxBlur: 6,   focusDepth: 0.35 },
    'f/5.6 Deep':         { maxBlur: 3,   focusDepth: 0.50 },
    'f/8.0 Sharp':        { maxBlur: 0.8, focusDepth: 0.70 },
    'f/11 Deep Focus':    { maxBlur: 0,   focusDepth: 1.00 }
  };
  return settings[dof] || { maxBlur: 0, focusDepth: 1.0 };
}
