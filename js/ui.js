...
function handleUpload(e) {
  ...
  reader.onload = function(evt) {
    ...
    img.onload = async function() { 
      ...
      // 🔥 NEW: Pre-process for sharpness and performance
      if (window.preprocessHeroImage) window.preprocessHeroImage(img);
...
function applyEffect(canvas, img, variation = {}) {
  ...
  // 1. BASE CHARACTER DIMENSIONS (Unified scaling)
  const { charScale, charH } = window.getSubjectMetrics(h, canvasRatio);
  let charW = charH * imgRatio;
  
  // 2. THE GROUND ANCHOR (Unified placement engine)
  const feetX = w / 2;
  const groundY = Math.max(0.1, Math.min(parseFloat(settings.groundPlaneY || S.groundPlaneY || 0.88), 0.95));
  const feetY = window.calculateVisibleFeetY(h, groundY, charH);
  ...
  if (charScaleMult > 1.0) {
    const shiftFactor = (charScaleMult - 1.0) * 0.5;
  ...
  // 🔥 FIX: Migration to Percentage-Based Stability (S.charXOffset is now 0.0-1.0)
  const driftX = (settings.charXOffset || S.charXOffset || 0) * w;
  const driftY = (settings.charYOffset || S.charYOffset || 0) * h;
  adjustedFeetY += driftY;
  const adjustedFeetX = feetX + driftX;
  ...
  if (S.backgroundPlate) {
    ...
    // 🔥 NEW: CALL THE GRADIENT DEPTH ENGINE (Unified Physics)
    const blurData = getBlurAmount(settings.dof || S.dof);
    let totalBlur = blurData.maxBlur * 1.5;
    let currentFocusDepth = blurData.focusDepth;
    let fgBlur = 0;
    ...
    if (S.rackFocus !== 0) {
      ...
      if (currentRack > 0) {
        // Pull to FG: Background gets more blurry, focus depth tightens
        totalBlur = Math.max(totalBlur, currentRack * 20); 
        currentFocusDepth = Math.max(0.05, currentFocusDepth * (1 - currentRack * 0.5));
      } else {
        // Pull to BG: Background gets sharper, foreground gets blurry
        totalBlur = Math.max(0, totalBlur + (currentRack * totalBlur)); 
    ...
    drawBackgroundWithSimpleDOF(ctx, S.backgroundPlate, bgX, bgY, scaledBgW, scaledBgH, focusPlaneY, totalBlur, timeFilter, settings.dutchAngle || 0, currentFocusDepth);
  }
  ...
  const charX = adjustedFeetX - (pCanvas.width / 2) + (moveX * 1.4); 
...
async function init() {
    ...
    try {
        ...
        S._isPopulated = false; // Prevents any auto-saves during init
        
        loadCustomPresets(); 
        if (typeof initDropzone === 'function') initDropzone(); 
        if (typeof loadLayout === 'function') loadLayout();

        // 🔐 CONFIG RECALL: Centrally managed via Persistence (with Legacy Migration)
        if (window.Persistence) {
            const config = window.Persistence.loadConfig();
            if (config) {
                console.log("📂 A-CAM: Configuration recovered from storage.");
                Object.keys(config).forEach(id => {
                    const el = document.getElementById(id);
                    if (el) el.value = config[id] || "";
                });
                
                // If this was a migration (first run with consolidated key), save it now
                if (!localStorage.getItem('acam_gcp_config')) {
                    window.Persistence.saveConfig(config);
                }
            }

            // Setup listeners to save config changes
            const configIds = ['acam_gemini_key', 'acam_fal_key', 'gcpProjectId', 'gcpRegion', 'gcpAccessToken'];
                ...
                if (el) {
                    el.addEventListener('input', () => {
                        ...
                        configIds.forEach(cId => {
                            ...
                            if (input && input.value) currentConfig[cId] = input.value;
                        });
                        window.Persistence.saveConfig(currentConfig);
                    });
                }
            });
        }
        ...
        if (window.Persistence) {
            ...
            if (session && session.settings) {
                console.log("📂 A-CAM: Restoring previous session settings...");
                Object.assign(S, session.settings);
                restoredSuccessfully = true;
            }
            
            // Load Assets (Hero, Background, Face, etc)
            const assetPromises = [];

            if (session.assets.heroImage) {
                console.log("👤 A-CAM: Recovering hero asset...");
                assetPromises.push(new Promise(resolve => {
                    const img = new Image();
                    img.onload = () => {
                        S.heroImage = img;
                        // 🔥 RE-SHARPEN UPON REFRESH: Rebuild the sharp cache from session image
                        if (window.preprocessHeroImage) window.preprocessHeroImage(img);
                    ...
                    img.onerror = () => { console.warn("Failed to load hero asset"); resolve(); };
            ...
            if (session.assets.backgroundPlate) {
                console.log("🏞️ A-CAM: Recovering background plate...");
                assetPromises.push(new Promise(resolve => {
                    const img = new Image();
                    img.onload = () => {
                    ...
                    img.onerror = () => { console.warn("Failed to load background asset"); resolve(); };
                    img.src = session.assets.backgroundPlate;
                }));
                restoredSuccessfully = true;
            }

            // Await critical assets before finalizing UI
            await Promise.all(assetPromises);

            if (session.assets.faceCloseup) { 
                console.log("🤳 A-CAM: Recovering face closeup...");
                ...
                if (preview) {
                    preview.src = session.assets.faceCloseup;
            ...
            if (session.assets.masterCharacterSheet) {
                console.log("📜 A-CAM: Recovering master identity sheet...");
                S.masterCharacterSheet = session.assets.masterCharacterSheet;
                restoredSuccessfully = true;

                const preview = document.getElementById('masterPreview');
                const status = document.getElementById('masterStatus');
                if (preview) {
                    preview.src = session.assets.masterCharacterSheet;
                    preview.style.display = 'block';
                }
                if (status) {
                    status.textContent = "LOCKED";
            ...
            if (session.assets.transformedStartFrame) { S.transformedStartFrame = session.assets.transformedStartFrame; restoredSuccessfully = true; }
            if (session.assets.transformedEndFrame) { S.transformedEndFrame = session.assets.transformedEndFrame; restoredSuccessfully = true; }

            // ⚓ SYNC ANTIGRAVITY ANCHORS
        ...
        if (typeof setShutterSpeed === 'function') setShutterSpeed(S.shutterSpeed || 'Standard');
        if (typeof updateLighting === 'function') updateLighting();
        
        // 🛡️ RE-SYNC ANCHOR BUCKETS (Visual State)
        if (window.Antigravity) {
            if (S.heroImage) window.Antigravity.setAnchor('Anatomy', true);
            if (S.faceCloseup) window.Antigravity.setAnchor('Identity', true);
            if (S.subjectOutfit || S.masterCharacterSheet) window.Antigravity.setAnchor('Outfit', true);
            if (S.sceneAnalyzed || S.backgroundPlate) window.Antigravity.setAnchor('Scene', true);
        }
        
        if (S.sceneAnalyzed) {
            document.getElementById('analyzedBadge')?.classList.add('active');
            document.getElementById('uploadZone')?.classList.add('analyzed');
        }

        // 🚀 FINAL RENDER: Ensure canvases are drawn
        setTimeout(() => {
        ...
        console.error("❌ A-CAM: Initialization Failed:", err);
    } finally {
        window.__ACAM_LOADING__ = false;
        
        // 🔥 THE SAFETY LOCK: Only enable auto-save if we actually restored a session
        // or if this is an explicitly new session (to be defined by user interaction)
        if (restoredSuccessfully) {
            S._isPopulated = true;
            console.log('✅ A-CAM: History restored. Auto-save active.');
        } else {
            console.log('✨ A-CAM: Fresh session started. Auto-save will activate on first change.');
        }
        
        console.log('🚀 A-CAM: Initialization Cycle Complete');
    }
}

/**
 * Helper to sync all UI inputs from the central state S
 */
function syncUIFromState() {
    // 1. Optics & Aperture
    if (document.getElementById('lensSelect')) {
        document.getElementById('lensSelect').value = S.lens || '35mm Standard';
        if (document.getElementById('lensValue')) document.getElementById('lensValue').textContent = (S.lens || '35mm').split(' ')[0];
    }
    if (document.getElementById('angleSelect')) {
        document.getElementById('angleSelect').value = S.angle || 'Eye Level';
        if (document.getElementById('angleValue')) document.getElementById('angleValue').textContent = (S.angle || 'Eye Level').split(' ')[0];
    }
    if (document.getElementById('dofSelect')) {
        document.getElementById('dofSelect').value = S.dof || 'f/5.6 Deep';
        if (document.getElementById('dofValue')) document.getElementById('dofValue').textContent = (S.dof || 'f/5.6').split(' ')[0];
    }
    
    // 2. Movement & Motion
    if (document.getElementById('movementSelect')) {
        document.getElementById('movementSelect').value = S.movement || 'Static';
        if (document.getElementById('movementValue')) document.getElementById('movementValue').textContent = (S.movement || 'Static').split(' ')[0];
    }
    if (document.getElementById('movementIntensity')) {
        document.getElementById('movementIntensity').value = S.movementIntensity || 50;
    }
    if (document.getElementById('rackFocusSlider')) {
        document.getElementById('rackFocusSlider').value = S.rackFocus || 0;
    }

    // 3. Environment & Lighting
    if (document.getElementById('lightingSelect')) {
        document.getElementById('lightingSelect').value = S.lighting || 'Natural Ambient';
        if (document.getElementById('lightingValue')) document.getElementById('lightingValue').textContent = (S.lighting || 'Natural').split(' ')[0];
    }
    if (document.getElementById('lightingIntensity')) {
        document.getElementById('lightingIntensity').value = S.lightingIntensity || 70;
    }
    if (document.getElementById('lightingIntensityValue')) {
        document.getElementById('lightingIntensityValue').textContent = (S.lightingIntensity || 70) + '%';
    }
    if (document.getElementById('timeSlider')) {
        document.getElementById('timeSlider').value = S.timeOfDay || 12;
    }
    if (document.getElementById('hazeSlider')) {
        document.getElementById('hazeSlider').value = S.hazeAmount || 0;
    }
    
    // 4. Identity & Mode Indicators
    if (document.getElementById('shutterValue')) {
        document.getElementById('shutterValue').textContent = S.shutterSpeed || 'Standard';
    }
    
    // 5. Textareas & Descriptions
    if (document.getElementById('subjectAction')) document.getElementById('subjectAction').value = S.subjectAction || "";
    if (document.getElementById('subjectOutfit')) document.getElementById('subjectOutfit').value = S.subjectOutfit || "";
    if (document.getElementById('actionTimeline')) document.getElementById('actionTimeline').value = S.actionTimeline || "";
    if (document.getElementById('sceneDescInput')) document.getElementById('sceneDescInput').value = S.sceneDescription || "";
}
...
function handleMasterUpload(e) {
    ...
    reader.onload = (evt) => {
        ...
        // ⚡ FAST-TRACK PERSISTENCE: Save immediately to IndexedDB to prevent loss on refresh
        if (window.Persistence) window.Persistence.saveIdentityImmediate(S);
    };
    reader.readAsDataURL(file);
}
...
function handleFaceUpload(e) {
    ...
    reader.onload = (evt) => {
        ...
        img.onload = () => {
            ...
            // ⚡ FAST-TRACK PERSISTENCE: Save immediately to IndexedDB
            if (window.Persistence) window.Persistence.saveIdentityImmediate(S);
        };
        img.src = evt.target.result;
    };
